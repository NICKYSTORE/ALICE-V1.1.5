...
async function pagi(alice, x, data) {
    ...
    return await alice.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(alice, x, data) {
    let row = [];
    let ment = [];
...
async function malam(alice, x, data) {
...
async function skill(alice, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await alice.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, alice, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await alice.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(alice, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(alice, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alice, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(alice, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alice, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(alice, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alice, data);
            break;
        } else {
            await skill(alice, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, alice, data);
}

async function run_vote(alice, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alice, data);
            break;
...
async function run_malam(alice, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alice, data);
            break;
...
async function run_pagi(alice, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, alice, data);
            break;
